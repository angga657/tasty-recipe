...
export default {
    ...
    actions: {
        async getRecipeData({ commit }) {
            try {
                const  { data } = await axios.get(
                    "https://vue-js-project-c8990-default-rtdb.firebaseio.com/recipes.json"
                )
                const arr = []
                for (let key in data) {
        ...
        async getRecipeDetail ({ commit }, payload) {
            try {
                const { data } = await axios.get(
                    `https://vue-js-project-c8990-default-rtdb.firebaseio.com/recipes/${payload}.json`
                )
        ...
        async deleteRecipe ({ dispatch, rootState }, payload) {
            try {
                ...
                (
                    `https://vue-js-project-c8990-default-rtdb.firebaseio.com/recipes/${payload}.json?auth=${rootState.auth.token}`
                );
        ...
        async updateRecipe({ dispatch, rootState }, { id, newRecipe}) {
            try {
                ...
                (
                 `https://vue-js-project-c8990-default-rtdb.firebaseio.com/recipes/${id}.json?auth=${rootState.auth.token}`, newRecipe   
               );
                await dispatch("getRecipeData");
            } catch (err) {
                console.log(err);
